<!-- i18n-source: README.md | sha256: d261b35641aa3941 | version: 2.15.1 | translated: 2026-09-18 | translation: f41198cf3a57467d -->
...
**能力七——查询时召回（v2.15.0）。** 上面这些都在*会话开始*时注入，那时还没有问题
存在，选择器只能按重要度和新近度排。而问题存在的那一刻，正是你把它发出去的那一刻，
所以 `UserPromptSubmit` 现在会拿你真正说的这句话去检索，并把命中写进会话：FTS5
BM25 加上本项目已有的 CJK 感知相似度，不需要向量、不需要索引服务。它**刻意保守**
——最多 3 条，相关度下限 0.45（在本仓库自己的 734 条记忆上标定：巧合最高只到
0.400），**够不上就一个字节都不写**，因为每轮都说话的通道是每轮都要交的税，还会
把人训练成跳过它。`memories.recall_count` 记录"真有人问起过"，与"盲排序选中了它"
分开计数，`/cc-mem inject-usage` 分两条通道报告。这是**词法**召回，把话说明白：
跨语言的同义词（`超时` / `timeout`）检索不到。
...
# ── 读取记忆 ───────────────────────────────────────────────────────────────
/cc-mem search "<query>"            FTS5 检索（trigram 分词，中文能切开；不足 3 个
                                    汉字的查询由子串兜底回答）
...
/cc-mem inject-usage [--window N] [--judge]
                                    ...
                                    哪些记忆真的进过上下文。--judge 加第 2 层：
                                    一次 LLM 调用判断送达的记忆有没有被真正用上
...
- **调试审查留下的未决项在案（v2.14.0）。** `docs/debug-pass-2026-09.md` 是
  证据记录：从不改动，且未翻译——它没有中文兄弟文件。它的发现里仍然开着的——
...
**那条有查询却空着的通道，和一个读不懂中文的检索。** 六条缺陷来自一个正在使用本
插件的项目，每条都带 `file:line` 和实测读数；每条都先在这里复现再动手，其中两条
最后被证明是同一个缺失特性的两半。

- **检索能看见中文了。** `memories_fts` 建表时没写 `tokenize=`，fts5 于是用了
  `unicode61`——它把汉字、假名、谚文都当作词内字符，从不切分，整句中文被索引成
  **一个** token。对着一条写着 `用户要求把超时设为三十秒` 的记忆实测：
  `MATCH '超时'` 返回 0，整句返回 1。现在索引是 `tokenize='trigram'`，由运行时
  探测选定（旧版 SQLite 没有 trigram 分词器），而旧版本建出来的索引会在打开时
  自行重建。
- **检索结果为空，永远不是一个答案。** `LIKE` 兜底以前只在 FTS 触发器缺失时才走，
  于是在健康状态下"索引什么都没找到"被当成事实返回——而 MCP 那一层把空结果集算作
  SUCCESS，模型因此被告知这个项目没有这条记忆。现在这条分支是无条件的。新闸门首跑
  还揪出两条：`search ""` 与 `search "\x00"` 各自返回了全部活跃行，而这两种拼法
  从 web viewer 和模型可调用的 `memory_search` 都够得着。
- **查询时召回——以及这个项目为什么不需要向量数据库。** 报告问的是"哪条写入路径
  产出了最没人用的记忆"，读数是 82.6% 的存量记忆从未被注入过。为此做的仪表
  （`memories.recall_count`）反而暴露了真正的发现：这个插件有**两个**可以把记忆送
  到 Claude 眼前的时刻，却只用了一个——唯一有用户查询的那个自动时刻只往数据库里写，
  stdout 什么都不打。见[能力七](#它做什么七项能力)。
- **`/cc-mem inject-usage` 真的算出了它承诺的信号。** 它的 docstring 宣称会测"确认句"，
  而没有任何一行代码在算。现在确认句是从收到上次注入的那个会话的 transcript 里量出来
  的，用的是钩子**发出**它时的同一个常量——一处要求，一处检测——而且是**三态**：
  `unmeasured` 绝不打印成 `no`。观察窗口是 `--window`，输出会说出它。**`--judge`**
  加上可选的第二层：一次 LLM 调用，读该会话自己的回复，逐条判断送达的记忆是
  `used` / `unused` / `unknown`。第 1 层免费且总是运行；第 2 层要花一次 API 调用，
  所以必须被明确要求，绝不默认——而 `unknown` 永远不会被渲染成 `unused`，因为我们
  这边的故障不是关于 Claude 行为的证据。
- **MERGE 不再毁掉它替换掉的那段文字**——两条改写分支现在都会归档旧行并把新行链回
  它，被取代的措辞因此仍可恢复。
- **漂移补救会收敛了。** 跑 `/cc-mem plan-check`（拒绝信息自己点名的那个补救）之后
  再碰一个文件，以前会在同一个 Stop 上重新武装同一条拦截；一次敏感 Bash 调用单独
  就能做到。现在一次守卫检查只对**它发生的那一轮**授予豁免，而补救文案把守卫排在
  **前**、计数器重置排在**后**，这样重置之后不会再有东西累加。另外 `plan-status` 与
  Stop 闸门读的是**同一个**判定函数，屏幕再也不会和拒绝信息互相矛盾。

本次发布自己的证伪用例里有两条首跑跑成了绿的，被修的是**检查**而不是用例：一个
"零字节"断言只有在配上一个会真的输出的阳性对照时才算证据；而一个由三重判定组成的
闸门，只有每一重都有单独探针时才算被量到。

v2.14.x 把一次 38 条发现的调试审查在根因上收干净了——项目的身份变成了它的数据库，
而不是数据库里那串路径——并收紧了四个一直在认证自己从未检查过的东西的闸门检查器。
v2.13.0 把每个项目的状态从 `memory/` 挪到了 `.ccm/`，首次写入时单向迁移，按内容而
不是按名字识别。v2.12.x 带来了背压触发的整理、步骤引用审计、CI 构建的 release，
