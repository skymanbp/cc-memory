...
def main():
    ...
    # ── v2.16.0 · step 10: core/prompts, ONE directive renderer, the fallback
    #    summary, MEMORY.md's notice, Recent by id, observer rows with a
    #    session, handshake Reads filtered ───────────────────────────────────
    from core import prompts as _p10_P
    from core import recall as _p10_recall
    _p10_src = (_REPO / "cc_memory" / "core" / "prompts.py").read_text(encoding="utf-8")
    assert "from core" not in _p10_src and "import core" not in _p10_src \
        and "neutralize" not in _p10_src, \
        "core/prompts.py holds text only: no package imports, no rendering"
    ...
    assert "resume_signals = set(RESUME_TRIGGERS)" in _p10_up_src \
        and '"接着做"' not in _p10_up_src, \
        "user_prompt.py must read the resume vocabulary from core.prompts, not re-spell it"
    _p10_rem = _hooks_ss._build_forced_reminder(mem_dir)
    assert all(f'"{tok}"' in _p10_rem for tok in _p10_P.RESUME_TRIGGERS if tok), \
        "the RESUME PROTOCOL must print every trigger"
    # (b) the fallback summary is ONE line and does not cover the topic
    from core.consolidate import _summarize_topic_fallback as _p10_fb
    _p10_line = _p10_fb("t", [{"content": f"fact {i} " + "x" * 80,
                               "importance": 3 + (i % 3)} for i in range(6)])
    assert _p10_line.startswith(_p10_P.FALLBACK_SUMMARY_PREFIX) \
        and "\n" not in _p10_line and len(_p10_line) <= 120 \
        and "6 memories" in _p10_line, repr(_p10_line)
    _p10_root = Path(tempfile.mkdtemp(prefix="cc-memory-p10-"))
    (_p10_root / _MEM).mkdir(parents=True)
    _p10_db = MemoryDB(_p10_root / _MEM / "memory.db")
    _p10_pid = _p10_db.upsert_project(str(_p10_root))
    for _i in range(3):
        _p10_db.insert_memory(_p10_pid, None, "config",
                              f"P10FACT {_i}: the deploy key rotates on day {_i}",
                              5, [], "keys")
    _p10_db.upsert_topic(_p10_pid, "keys",
                         _p10_fb("keys", _p10_db.get_memories_by_topic(_p10_pid, "keys")))
    _p10_tt, _p10_names = _hooks_ss._build_topics_layer(_p10_db, _p10_pid, 4000)
    assert "keys" not in _p10_names and "3 memories" in _p10_tt, \
        "a fallback-summarised topic must render but must NOT count as covered"
    _p10_ct, _p10_ids = _hooks_ss._build_critical_layer(_p10_db, _p10_pid, 4000, _p10_names)
    assert len(_p10_ids) == 3, (
        f"the critical rows of a fallback topic must still be injected "
        f"({len(_p10_ids)} of 3); v2.15.2 dropped them as 'covered' by a "
        f"summary that stated nothing")
    _p10_db.upsert_topic(_p10_pid, "keys", "The deploy key rotates daily; the schedule is fixed.")
    _p10_tt2, _p10_names2 = _hooks_ss._build_topics_layer(_p10_db, _p10_pid, 4000)
    assert "keys" in _p10_names2 and \
        _hooks_ss._build_critical_layer(_p10_db, _p10_pid, 4000, _p10_names2)[1] == set(), \
        "a REAL summary still covers its topic"
    # (c) MEMORY.md: a three-line notice, no maintainer tail
    from llm.memory_writer import _render_memory_index as _p10_rmi
    _p10_md = _p10_rmi(_p10_db, _p10_pid, _p10_root / _MEM)
    _p10_cm = _p10_md[_p10_md.index("<!--"):_p10_md.index("-->")]
    assert _p10_cm.count("\n") <= 3 and "DO NOT EDIT THIS FILE BY HAND" in _p10_cm \
        and "/cc-mem add" in _p10_cm, repr(_p10_cm)
    assert "python -m cc_memory" not in _p10_md, "the maintainer-only tail is gone"
    # (d) ONE directive renderer behind PLAN.md and the injection layer
    from core import plan as _p10_plan
    _p10_db.upsert_directive(_p10_pid, "ship-weekly", demand="ship every Friday",
                             quote="ship it weekly")
    _p10_rows = _p10_db.list_directives(_p10_pid, status="active")
    _p10_inj = _p10_plan.render_directive_lines(_p10_rows, style="inject")
    _p10_pl = _p10_plan.render_directive_lines(_p10_rows, style="plan")
    assert [s for s, _ in _p10_inj] == ["ship-weekly"] == [s for s, _ in _p10_pl]
    assert "ship-weekly (stated ×1): ship every Friday" in _p10_inj[0][1] \
        and "**ship-weekly**" in _p10_pl[0][1], (_p10_inj, _p10_pl)
    _p10_lay, _p10_slugs = _hooks_ss._build_directives_layer(_p10_db, _p10_pid, 4000)
    assert _p10_inj[0][1] in _p10_lay and _p10_slugs == ["ship-weekly"], _p10_lay
    assert _p10_pl[0][1] in "\n".join(_p10_plan._render_directives_section(_p10_rows))
    _p10_dl = _ss_src[_ss_src.index("def _build_directives_layer("):
                      _ss_src.index("def _build_footer(")]
    assert "stated ×" not in _p10_dl, \
        "the injection layer must not carry a directive format of its own"
    # (e) Recent is the NEWEST rows, by id
    _p10_db.insert_memory(_p10_pid, None, "note", "OLD but important: keep the old key",
                          5, [], "")
    for _i in range(25):
        _p10_db.insert_memory(_p10_pid, None, "note",
                              f"NEWER fact number {_i} about the exporter", 3, [], "")
    _p10_rec = _p10_db.get_recent_memories(_p10_pid, sessions_back=3,
                                           min_importance=3, limit=20)
    assert len(_p10_rec) == 20 and all("NEWER" in m["content"] for m in _p10_rec) \
        and [m["id"] for m in _p10_rec] == sorted((m["id"] for m in _p10_rec),
                                                   reverse=True), (
        "Recent must be the newest rows (v2.15.2 ranked by importance first, so "
        "one old important row displaced the newest)")
    # (f) the observer attaches its rows to the session and skips the
    #     handshake Read; PreCompact reuses the claim row
    for _i in range(3):
        _p10_db.insert_observation(_p10_pid, "p10-sess", "Edit", f"src/f{_i}.py", "")
    _p10_db.insert_observation(_p10_pid, "p10-sess", "Read",
                               str(_p10_root / _MEM / "PROGRESS.md"), "(file content)")
    _p10_calls = []
    # `_be8` was rebound to `build_extraction` by an import above; bind the
    # modules afresh (stop.py imports both lazily inside _observer_evaluate,
    # so patching the module attributes is what the hook sees)
    import llm.ccl_backend as _p10_be
    import core.auth as _p10_auth
    _p10_saved = (_p10_be.call_llm, _p10_auth.get_api_key)
    _p10_be.call_llm = (lambda *a, **k: _p10_calls.append(a[1]) or
                     '[{"category":"note","content":"observer fact about the exporter port 9100",'
                     '"importance":3,"topic":"exporter"}]')
    # why: a placeholder, not a credential — call_llm is stubbed above, so
    # nothing built from it reaches the network
    _p10_auth.get_api_key = lambda: ("placeholder-key", "env")
    try:
        _p10_n = _sh1_stop._observer_evaluate(str(_p10_root), "p10-sess",
                                              _p10_root / _MEM, db=_p10_db)
    finally:
        _p10_be.call_llm, _p10_auth.get_api_key = _p10_saved
    assert _p10_n == 1 and len(_p10_calls) == 1, (_p10_n, len(_p10_calls))
    assert "PROGRESS.md" not in _p10_calls[0] and "src/f1.py" in _p10_calls[0], \
        f"the handshake Read must not be fed to the observer: {_p10_calls[0][-300:]!r}"
    _p10_row = [m for m in _p10_db.get_all_active_memories(_p10_pid)
                if "observer fact" in m["content"]][0]
    _p10_sid = _p10_db.session_row_for(_p10_pid, "p10-sess")
    assert _p10_sid is not None and _p10_row["session_id"] == _p10_sid, \
        f"an observer row must belong to its session ({_p10_row['session_id']} vs {_p10_sid})"
    _p10_sid2 = _p10_db.claim_session(project_id=_p10_pid, claude_session_id="p10-sess",
                                      trigger_type="auto", msg_count=5,
                                      archive_path="", brief_summary="")
    assert _p10_sid2 == _p10_sid, "PreCompact must reuse the observer's incomplete claim row"
    _p10_db.mark_session_complete(_p10_sid2)
    assert _p10_db.claim_session(project_id=_p10_pid, claude_session_id="p10-sess",
                                 trigger_type="auto", msg_count=5,
                                 archive_path="", brief_summary="") != _p10_sid, \
        "a completed row is never reused: each compaction is its own session row"
    # (g) the handshake Read is neither a file touched nor a row PreCompact feeds
    from core.extractor import files_from_observations as _p10_ffo, \
        is_handshake_read as _p10_ihr
    _p10_obs = sorted(_p10_db.get_recent_observations(_p10_pid, limit=10),
                      key=lambda o: o["id"])
    _p10_reads, _p10_mods = _p10_ffo(_p10_obs)
    assert not any("PROGRESS.md" in p for p in _p10_reads) and len(_p10_mods) == 3, \
        (_p10_reads, _p10_mods)
    assert _p10_ihr("Read", "D:/x/.ccm/MEMORY.md") and _p10_ihr("Read", "/p/memory/PLAN.md") \
        and not _p10_ihr("Read", "/p/docs/PROGRESS.md") \
        and not _p10_ihr("Edit", "/p/.ccm/PROGRESS.md")
    _p10_pc_spec = _ilu.spec_from_file_location(
        "_pc_p10", _REPO / "cc_memory" / "hooks" / "pre_compact.py")
    _p10_pc = _ilu.module_from_spec(_p10_pc_spec)
    _p10_pc_spec.loader.exec_module(_p10_pc)
    _p10_fed, _p10_chars, _p10_unfed = _p10_pc._observations_to_feed(_p10_obs, 0)
    assert len(_p10_unfed) == 4 and len(_p10_fed) == 3 \
        and all(o["tool_name"] == "Edit" for o in _p10_fed), (len(_p10_unfed), len(_p10_fed))
    shutil.rmtree(_p10_root, ignore_errors=True)
    print("[OK] v2.16.0 step 10: core.prompts is text-only and the one resume "
          "vocabulary; the fallback summary is one line and covers nothing; "
