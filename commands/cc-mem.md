...
| `inject-show [--templates]` | Show exactly what the last SessionStart injected (ground truth); `--templates` lists every Claude-visible text template in `core/prompts.py` with its size (v2.16.0) |
| `inject-usage [--window N] [--judge]` | Deterministic signals, **each naming the window it was measured over** — a count over an unstated window reads as evidence of absence and is not. Counts Read-tool observations targeting `PROGRESS.md` / `MEMORY.md` across the last `--window` observation rows (default 200; widen it when a long session may have buried the Read past them), and reports whether the **handoff ack was stated** — the sentence `SessionStart`'s forced reminder demands, matched through the same constant that emits it (`core.progress.ACK_TEMPLATE`), in the transcript of the session that received the last injection. The ack is **tri-state**: `yes` / `no` / `unmeasured`, and `unmeasured` (no injection recorded, no session id in the manifest, no transcript on disk) is never rendered as `no`. Quoting the reminder is not stating the ack, and neither is a tool argument that contains it. Per-memory `#id` matching is deliberately **not** done — it is feasible, since the timeline layer renders `#<id>` for every entry past the fifth, but it would need the ids that layer actually spelled rather than the whole manifest, or it measures the injection's shape and reports it as Claude's behaviour. **`--judge` adds layer 2**, off by default: one LLM call that reads Claude's own replies from that session and judges, per delivered memory, whether it was `used` / `unused` / `unknown`. Layer 1 above is deterministic, free and always runs; layer 2 costs an API call, which is why it is asked for rather than assumed. `unknown` — no credential, a refused call, an unparsable answer, an id the judge skipped — is never printed as `unused`: an outage of ours is not evidence about Claude |
