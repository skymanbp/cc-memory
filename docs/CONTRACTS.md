...
0b. Everything from here through step 5 runs inside ONE `BEGIN IMMEDIATE`
   transaction — `db.reconcile_upsert` (core/db.py:1623-1799). `upsert_smart`
...
| `SessionStart` retroactive save | `upsert_batch(db, pid, sid, memories, memory_dir=memory_dir)` — un-saved prior sessions (`hooks/session_start.py:1131`) |
...
| Dashboard UI "Init Project" scan | `upsert_batch(db, pid, None, batch, memory_dir=memory_dir)` (`ui/dashboard.py:2310`) |
...
`core/progress.py:774-792`, called from `hooks/pre_compact.py:595`).
...
`db.get_recent_sessions` (`core/progress.py:501`; `core/db.py:3056-3110`):
...
| `status_blocked` | TEXT | Explicit `patch_progress(status_blocked=...)` — no in-tree caller does this today; it is an API for external tooling. A repo-wide grep finds only the schema default (`core/db.py:2937-2976,853`), the empty seed (`core/progress.py:286`) and the read (`core/progress.py:286`) |
| `open_todos` | JSON | PreCompact `extract_latest_todo_state(window)` via `ext["latest_todos"]` (`core/extractor.py:478-513,558`; `pre_compact.py:630,656`) → SessionStart tier-3 prior-transcript mine (`session_start.py:1055`) → LAST RESORT `session_summary.next_steps` split by `;` (`session_start.py:1055`). Only non-`completed` todos are kept (`progress.py:286`) |
| `plan` | TEXT | `session_summaries.next_steps` — sourced from the latest TodoWrite pending items if any, else from LLM-extracted `task` memories (`pre_compact.py:462-468`); propagated at `progress.py:288`, filled-if-empty at `session_start.py:1055` |
| `critical_context` | JSON | Top 10 memories with importance ≥ 4, content truncated to 200 chars (`progress.py:107-113`; `session_start.py:1056`) |
| `files_touched` | JSON | `observations` table (`pre_compact.py:446-453` → `progress.py:128-134`; Stop per-turn patch `stop.py:193-211`; SessionStart tier-2C `session_start.py:1056`) → tier-3 prior-transcript `extract_file_changes` (`session_start.py:1056`) |
| `transcript_ptr` | TEXT | PreCompact `transcript_path` resolved absolute (`pre_compact.py:807`) → tier-3 `find_latest_transcript(cwd, exclude_session_id=...)` (`session_start.py:1019`) |
| `updated_at` | TEXT | ISO timestamp, stamped by `upsert_progress` / `patch_progress` (`db.py:2769-2845`, `:937-943`) |
| `trigger_type` | TEXT | "auto" \| "manual" (PreCompact passes the host's own trigger string through — `pre_compact.py:812,492`; `"precompact"` is only `collect_progress_state`'s default kwarg at `progress.py:200-260` and is always overridden) \| "stop" (`stop.py:778`) \| "user_prompt" \| "resume_request" (`user_prompt.py:442`) \| "session_start_refresh" (`session_start.py:1081`) |
| `current_session_id` | TEXT | `db.tag_progress_session` only (`db.py:3030-3054`) — tagged by PreCompact (`pre_compact.py:812`), Stop (`stop.py:778`), SessionStart (`session_start.py:1081`), UserPromptSubmit (`user_prompt.py:442`) |
...
`user_prompt.py:434`, `session_start.py:1215`, `cli/mem.py:1465`,
`mcp/server.py:243`.
...
1. **PreCompact** (full rewrite, all fields fresh):
   ...
   - `db.tag_progress_session(...)` runs FIRST so the tag survives
     (`pre_compact.py:812`; see the preservation logic at `db.py:3030-3054`).
   - `db.upsert_progress(**all_fields)` overwrites the row (`pre_compact.py:807`).
...
3. **UserPromptSubmit** (the session's first NON-scaffolding prompt, once):
   ...
   - The seed happens ONCE per session, recorded by a `cc_mem_seeded_` temp
     ...
     request.
   - `db.tag_progress_session(...)` (`user_prompt.py:442`) then
...
4. **SessionStart refresh** (every session start, tier-2/3 fallback):
   ...
   - EMPTY is not NEVER WRITTEN. `[]` is falsy, so an `open_todos` PreCompact
     ...
     (`progress_was_fully_written`, `session_start.py:962-988`), the mined
   ...
   - On `source="compact"` / `"resume"` tier 3 mines the CURRENT transcript
     (`tier3_exclusion`, `session_start.py:1011-1026`): the session id is unchanged
   ...
   - Fill-only-empty: never overwrites a non-empty field upstream wrote
     (contract stated at `session_start.py:967`).
...
The block is emitted only when PROGRESS.md exists, and since v2.16.0 it demands
that ONE file: MEMORY.md was a second mandatory Read of an index whose facts the
injection's own layers already carry, so every session paid a tool call and a
context window's worth of index for nothing the handoff needed
(`session_start.py:_build_forced_reminder`; `/cc-mem inject-usage` still counts
a Read of MEMORY.md). No PROGRESS.md means no block — a reminder that demanded
only the index was the whole handshake on a project that had never compacted.
...
Hardcoded defaults in `core/plan.py:1434-1444` (`turn_threshold=8`,
