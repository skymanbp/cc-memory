...
| `Stop` observer | `upsert_batch(db, pid, None, observer_list, memory_dir)` (`hooks/stop.py:279`) |
...
| `trigger_type` | TEXT | "auto" \| "manual" (PreCompact passes the host's own trigger string through — `pre_compact.py:749,492`; `"precompact"` is only `collect_progress_state`'s default kwarg at `progress.py:200-260` and is always overridden) \| "stop" (`stop.py:386`) \| "user_prompt" \| "resume_request" (`user_prompt.py:193`) \| "session_start_refresh" (`session_start.py:825`) |
| `current_session_id` | TEXT | `db.tag_progress_session` only (`db.py:2264-2288`) — tagged by PreCompact (`pre_compact.py:749`), Stop (`stop.py:386`), SessionStart (`session_start.py:825`), UserPromptSubmit (`user_prompt.py:193`) |
...
All six `write_progress_md` call sites: `pre_compact.py:751`, `stop.py:325`,
...
1. **PreCompact** (full rewrite, all fields fresh):
   ...
   - `db.tag_progress_session(...)` runs FIRST so the tag survives
     (`pre_compact.py:749`; see the preservation logic at `db.py:2264-2288`).
