...
| `Stop` observer | `upsert_batch(db, pid, None, observer_list, memory_dir)` (`hooks/stop.py:358`) |
...
| Dashboard UI "Add Memory" | `upsert_smart(...)` + `regenerate_memory_index(...)` — routed since v2.2 (`ui/dashboard.py:1674,956`). `ui/dashboard.py` contains no `db.insert_memory` call. |
| Dashboard UI "Save Session" | `upsert_batch(...)` (`ui/dashboard.py:2256`) |
| Dashboard UI "Init Project" scan | `upsert_batch(db, pid, None, batch, memory_dir=memory_dir)` (`ui/dashboard.py:2256`) |
...
(`cli/mem.py:885`), the contract is working. Zero is fine (no facts have
...
`db.get_recent_sessions` (`core/progress.py:324`; `core/db.py:2395-2449`):
...
| `status_blocked` | TEXT | Explicit `patch_progress(status_blocked=...)` — no in-tree caller does this today; it is an API for external tooling. A repo-wide grep finds only the schema default (`core/db.py:2328-2367,853`), the empty seed (`core/progress.py:276`) and the read (`core/progress.py:276`) |
...
| `trigger_type` | TEXT | "auto" \| "manual" (PreCompact passes the host's own trigger string through — `pre_compact.py:750,492`; `"precompact"` is only `collect_progress_state`'s default kwarg at `progress.py:200-260` and is always overridden) \| "stop" (`stop.py:535`) \| "user_prompt" \| "resume_request" (`user_prompt.py:213`) \| "session_start_refresh" (`session_start.py:890`) |
| `current_session_id` | TEXT | `db.tag_progress_session` only (`db.py:2369-2393`) — tagged by PreCompact (`pre_compact.py:750`), Stop (`stop.py:535`), SessionStart (`session_start.py:890`), UserPromptSubmit (`user_prompt.py:213`) |
...
All six `write_progress_md` call sites: `pre_compact.py:752`, `stop.py:474`,
`user_prompt.py:205`, `session_start.py:1013`, `cli/mem.py:1392`,
`mcp/server.py:243`.
...
1. **PreCompact** (full rewrite, all fields fresh):
   ...
   - `db.tag_progress_session(...)` runs FIRST so the tag survives
     (`pre_compact.py:750`; see the preservation logic at `db.py:2369-2393`).
...
`.ccm/.plan_raw.md` (`cli/mem.py:1782`).
