<!-- i18n-source: CONTRACTS.md | sha256: 227987d4915f2d4f | version: 2.15.2 | translated: 2026-09-24 | translation: e12968da6014abf5 -->
...
| 保存路径 | 入口函数 |
|-----------|---------------|
| `PreCompact` 钩子 | `upsert_batch(db, pid, sid, extracted_list, memory_dir)`（`hooks/pre_compact.py:716`） |
| `Stop` 观察者 | `upsert_batch(db, pid, None, observer_list, memory_dir)`（`hooks/stop.py:492`） |
...
`migrate_legacy_handoff`，`core/progress.py:735-753`，从 `hooks/pre_compact.py:588`
调用）。
...
| `current_request` | TEXT | UserPromptSubmit 首条非脚手架提示、每会话一次（`user_prompt.py:330`）→ PreCompact 的 `_first_user_request(window.head)`（`pre_compact.py:335-392`）—— 它会扫描至多 200 条记录，越过开头的 `queue-operation` / `attachment` 元数据行，并跳过内容为空的 user 行（`pre_compact.py:335-392`，v2.4.2）→ `session_summaries.request`（`progress.py:244`） |
...
| `transcript_ptr` | TEXT | PreCompact 解析为绝对路径的 `transcript_path`（`pre_compact.py:794`）→ 第 3 级 `find_latest_transcript(cwd, exclude_session_id=...)`（`session_start.py:940`） |
| `updated_at` | TEXT | ISO 时间戳，由 `upsert_progress` / `patch_progress` 打戳（`db.py:2769-2845`、`:937-943`） |
| `trigger_type` | TEXT | "auto" \| "manual"（PreCompact 把宿主自己的触发字符串原样透传 —— `pre_compact.py:84,492`；`"precompact"` 只是 `collect_progress_state` 在 `progress.py:200-260` 的默认关键字参数，且总会被覆盖）\| "stop"（`stop.py:731`）\| "user_prompt" \| "resume_request"（`user_prompt.py:392`）\| "session_start_refresh"（`session_start.py:1002`） |
| `current_session_id` | TEXT | 只由 `db.tag_progress_session` 写入（`db.py:2958-2982`）—— 由 PreCompact（`pre_compact.py:799`）、Stop（`stop.py:731`）、SessionStart（`session_start.py:1002`）、UserPromptSubmit（`user_prompt.py:392`）打标签 |
...
`write_progress_md` 调用点：`pre_compact.py:801`、`stop.py:635`、`user_prompt.py:384`、
...
1. **PreCompact**（整篇重写，所有字段全新）：
   ...
   - `db.tag_progress_session(...)` **先**运行，这样标签才能存活
     （`pre_compact.py:799`；保留逻辑见 `db.py:2958-2982`）。
   - `db.upsert_progress(**all_fields)` 覆盖整行（`pre_compact.py:794`）。
...
3. **UserPromptSubmit**（本会话首条**非脚手架**提示，仅一次）：
   ...
   - 先 `db.tag_progress_session(...)`（`user_prompt.py:392`），再
...
硬编码的默认值在 `core/plan.py:1339-1355`（`turn_threshold=8`、`edit_threshold=12`）；
