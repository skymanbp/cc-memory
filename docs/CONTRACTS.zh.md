<!-- i18n-source: CONTRACTS.md | sha256: 44de42b704269d36 | version: 2.9.0 | translated: 2026-08-09 -->
...
- 不要自己撸一套 `"SELECT content FROM memories ..."` 去重。那正是
  `db.find_by_hash`（`core/db.py:1910-1918`）和写入器的 `_find_similar`
...
读取 `sessions` / `session_summaries` 表（`core/progress.py:301`；`core/db.py:2248-2302`）：

| 列 | 类型 | 主来源 · 兜底 |
|--------|------|---------------------------|
| `project_id` | INTEGER PK | `upsert_project` |
| `current_request` | TEXT | UserPromptSubmit 第 1 回合（`user_prompt.py:166`）→ PreCompact 的 `_first_user_request(window.head)`（`pre_compact.py:269-311`）—— 它会扫描至多 200 条记录，越过开头的 `queue-operation` / `attachment` 元数据行，并跳过内容为空的 user 行（`pre_compact.py:269-311`，v2.4.2）→ `session_summaries.request`（`progress.py:241`） |
...
| `status_blocked` | TEXT | 显式的 `patch_progress(status_blocked=...)` —— 今天树内没有任何调用方这样做；它是留给外部工具的 API。全仓库 grep 只能找到 schema 默认值（`core/db.py:2223-2262,853`）、空播种（`core/progress.py:253`）和读取处（`core/progress.py:253`） |
| `open_todos` | JSON | PreCompact 经 `ext["latest_todos"]` 调用 `extract_latest_todo_state(window)`（`core/extractor.py:478-513,558`；`pre_compact.py:630,656`）→ SessionStart 第 3 级：挖掘上一次会话的 transcript（`session_start.py:880`）→ **最后手段**：把 `session_summary.next_steps` 按 `;` 切分（`session_start.py:880`）。只保留非 `completed` 的 todo（`progress.py:253`） |
| `plan` | TEXT | `session_summaries.next_steps` —— 若有最新 TodoWrite 的 pending 项则取自它，否则取自 LLM 抽取出的 `task` 类记忆（`pre_compact.py:462-468`）；在 `progress.py:255` 传播，在 `session_start.py:880` 按“空则填”补齐 |
...
| `trigger_type` | TEXT | "auto" \| "manual"（PreCompact 把宿主自己的触发字符串原样透传 —— `pre_compact.py:67,492`；`"precompact"` 只是 `collect_progress_state` 在 `progress.py:200-260` 的默认关键字参数，且总会被覆盖）\| "stop"（`stop.py:395`）\| "user_prompt" \| "resume_request"（`user_prompt.py:214`）\| "session_start_refresh"（`session_start.py:823`） |
| `current_session_id` | TEXT | 只由 `db.tag_progress_session` 写入（`db.py:2264-2288`）—— 由 PreCompact（`pre_compact.py:757`）、Stop（`stop.py:395`）、SessionStart（`session_start.py:823`）、UserPromptSubmit（`user_prompt.py:214`）打标签 |
| `session_started_at` | TEXT | `db.tag_progress_session` —— 只在存储的 sid 发生变化时重置；`upsert_progress` 在整篇重写时会把这两个字段一并保留（`db.py:2264-2288`） |
...
`write_progress_md` 调用点：`pre_compact.py:759`、`stop.py:323`、`user_prompt.py:207`、
`session_start.py:946`、`cli/mem.py:1277`、`mcp/server.py:742`。
...
1. **PreCompact**（整篇重写，所有字段全新）：
   ...
   - `db.tag_progress_session(...)` **先**运行，这样标签才能存活
     （`pre_compact.py:757`；保留逻辑见 `db.py:2264-2288`）。
...
3. **UserPromptSubmit**（仅第 1 回合）：
   - 先 `db.tag_progress_session(...)`（`user_prompt.py:214`），再
...
`cmd_plan_clear`（`cli/mem.py:1612-1641`）会拒绝并以 1 退出（`:788-798`）：
