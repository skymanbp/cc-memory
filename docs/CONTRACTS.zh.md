<!-- i18n-source: CONTRACTS.md | sha256: 09b19f9eba4db210 | version: 2.14.0 | translated: 2026-09-01 | translation: 7ec1adb8d00055ab -->
...
| `SessionStart` 追溯保存 | `upsert_batch(db, pid, sid, memories, memory_dir=memory_dir)` —— 处理此前未保存的会话（`hooks/session_start.py:1144`） |
...
| `open_todos` | JSON | PreCompact 经 `ext["latest_todos"]` 调用 `extract_latest_todo_state(window)`（`core/extractor.py:478-513,558`；`pre_compact.py:630,656`）→ SessionStart 第 3 级：挖掘上一次会话的 transcript（`session_start.py:880`）→ **最后手段**：把 `session_summary.next_steps` 按 `;` 切分（`session_start.py:880`）。只保留非 `completed` 的 todo（`progress.py:276`） |
...
| `critical_context` | JSON | importance ≥ 4 的前 10 条记忆，内容截断到 200 字符（`progress.py:107-113`；`session_start.py:881`） |
| `files_touched` | JSON | `observations` 表（`pre_compact.py:446-453` → `progress.py:128-134`；Stop 每回合打补丁 `stop.py:193-211`；SessionStart 第 2C 级 `session_start.py:881`）→ 第 3 级：对上一次会话 transcript 跑 `extract_file_changes`（`session_start.py:881`） |
| `transcript_ptr` | TEXT | PreCompact 解析为绝对路径的 `transcript_path`（`pre_compact.py:751`）→ 第 3 级 `find_latest_transcript(cwd, exclude_session_id=...)`（`session_start.py:952`） |
| `updated_at` | TEXT | ISO 时间戳，由 `upsert_progress` / `patch_progress` 打戳（`db.py:2389-2465`、`:937-943`） |
| `trigger_type` | TEXT | "auto" \| "manual"（PreCompact 把宿主自己的触发字符串原样透传 —— `pre_compact.py:71,492`；`"precompact"` 只是 `collect_progress_state` 在 `progress.py:200-260` 的默认关键字参数，且总会被覆盖）\| "stop"（`stop.py:561`）\| "user_prompt" \| "resume_request"（`user_prompt.py:213`）\| "session_start_refresh"（`session_start.py:896`） |
| `current_session_id` | TEXT | 只由 `db.tag_progress_session` 写入（`db.py:2589-2613`）—— 由 PreCompact（`pre_compact.py:756`）、Stop（`stop.py:561`）、SessionStart（`session_start.py:896`）、UserPromptSubmit（`user_prompt.py:213`）打标签 |
...
`write_progress_md` 调用点：`pre_compact.py:758`、`stop.py:500`、`user_prompt.py:205`、
`session_start.py:1019`、`cli/mem.py:1416`、`mcp/server.py:745`。
...
1. **PreCompact**（整篇重写，所有字段全新）：
   ...
   - `db.tag_progress_session(...)` **先**运行，这样标签才能存活
     （`pre_compact.py:756`；保留逻辑见 `db.py:2589-2613`）。
