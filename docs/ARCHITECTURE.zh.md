...
cc-memory/
├── .claude-plugin/
│   ├── plugin.json              ← 插件清单（version = 当次发布号）
...
`hooks/hooks.json` 声明了**横跨 5 个 Claude Code 事件的 6 条钩子命令**<!--ce:hooks-->——
...
| `PostToolUse` | [`cc_memory/hooks/post_tool_use.py`](../cc_memory/hooks/post_tool_use.py) | 8s | **先**做实时计划集成，且所有模式一视同仁：`ExitPlanMode` → `plan_active.raw`，`TodoWrite` → 机械式步骤同步，`Edit`/`Write`/`MultiEdit`/`NotebookEdit` → 漂移计数器 +1，敏感 Bash 调用 → +20。**然后**才为被观测的工具调用向 `observations` 插入一行（模式白名单 / 跳过列表——`core.modes.should_observe`）。不调用 LLM。端到端实测约 180-290 ms，其中约 75-120 ms 是解释器启动。自 v2.16.0 起由 matcher 绑定：`hooks/hooks.json` 声明的是 `core.modes.HOOK_TOOL_MATCHER`——一条锚定的选择式正则，恰好列出这个钩子真正处理的工具（各模式的 `observe_tools` ∪ 计划相关的几条支路）；以前它绑定每一次工具调用，对其中大多数只是提前返回。 |
...
- **背压是第三个触发器（v2.12.0）。** 会话间隔假设压缩会发生；一个只在短会话里
  ...
  `core.consolidate.consolidation_backlog` 读取标记里的 `last_memory_id`
  行号水位线，在 50 条未整理行、或 7 天且新增 ≥ 10 行时判定到期。Stop 钩子
  每回合探测一次（一条 COUNT 查询，`stop.py:_maybe_kick_consolidation`），并以
  **分离进程**方式拉起同一个异步工作者（`consolidate_async.py --cwd`）；工作者
  会在整理锁之下重新检查判据，因此竞态的拉起是空操作，而 `.consolidation.kick`
  冷却（10 分钟）为反复失败的工作者的重启设了上界。标记 I/O 与手动 CLI 路径
  共享同一实现（`core.consolidate.read_consolidation_marker` /
  `write_consolidation_marker`），后者现在也会盖章标记——另外
  `/cc-mem consolidate --deep` 会循环语义去重裁判直到跑干
...
SessionStart：
  ...
  发出：<system-reminder>
          You MUST Read .ccm/PROGRESS.md before（自 v2.16.0 起只要求这一个
...
无法中断一条已经在飞的腿，所以一个卡住的 socket 就是 40 s 撞 15 s 的强杀。三个钩子 <!--ce:hooks:subset-->
...
├── .retro.lock                  分离的追溯保存工作进程的锁（v2.16.0）
...
| `ui/web_viewer.py` | 不会，只读 | `print` |
| `skills/ccm-load`、`skills/save-memories` | 会 | `print` |

两处容易做错、而且确实做错过的细节：

- **`announce` 是参数，不是 `print`。** MCP 服务器在 stdout 上讲 JSON-RPC，往那里写一行
  重定向通知会破坏帧格式、直接搞死会话；它传的是 `_log.info`。所有面向人的入口都传打印器，
  因为显式的 `--project` 是一条指令，悄悄拿它做别的事比那个 bug 更糟。
- **退出名单一律在锚定之前检查。** 锚定是那个可能**向上移动**的步骤；先锚定就会把被排除的
  子目录解析到未被排除的父目录，从而服务一个用户已经选择退出的项目。`test_surfaces` §7
  在源码层面对全部六个 hook<!--ce:hooks--> 断言了这个顺序。

`anchor_project` 的比较**两边都 resolve**。当答案就是输入本身时，`project_root` 返回调用方
...
六个 hook<!--ce:hooks-->，以及"每个 hook 都在退出开关**之后**解析"这条源码级规则。
...
路径；现在打印的是确实存在的 `TARGET_DIR/cli/mem.py`（`installer.py:72`）。
