...
SessionStart:
  ...
  emit: <system-reminder>
          You MUST Read .ccm/PROGRESS.md before (the one file demanded
...
├── .retro.lock                  the detached retroactive-save worker's lock (v2.16.0)
...
with a 60 s horizon; `.llm_backoff.json` ←
