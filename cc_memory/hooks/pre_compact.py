...
Output:
  stderr suppressed.
  stdout: nothing (v2.16.0). The one-line status goes to the log: a
    PreCompact hook's stdout is shown in the transcript view only and
    never reached the compacted context it was printed for.
...
def main():
    ...
    # worth being able to explain afterwards. No stdout, no artifact: an
    # excluded project must produce nothing at all.
    ...
    try:
        ...
        # No `memory_dir`: the batch must not render MEMORY.md here (v2.16.0,
        # A7). The index also shows the session count and the top keywords,
        # and both are written BELOW this line, so the render that counts is
        # the one at the end of this function — the first render was a
        # second full read of the table for a file the second overwrote.
        counts = upsert_batch(db, project_id, session_id, extracted)
        ...
        # THE one MEMORY.md render of a compaction (v2.16.0, A7): after the
        # memories, the keywords, the session summary and the cleanup, so
        # the index reflects all of them. `upsert_batch` above was told not
        # to render.
        ...
        # No stdout (v2.16.0, A7). The "visible status for the next session"
        # this used to print reached nobody: a PreCompact hook's stdout is
        # shown in the transcript view only, never in the compacted context
        # (verified against the hooks documentation, docs/plans/2026-09-24).
        # The counts are in the log line above.
