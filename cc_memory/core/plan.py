...
from core.prompts import PLAN_MD_HEADER
...
_PLAN_MD_HEADER = list(PLAN_MD_HEADER)   # the text lives in core.prompts (v2.16.0)
...
def _render_directives_section(directives: Optional[List[Dict]]) -> List[str]:
    ...
    out += [line for _slug, line in render_directive_lines(rows, style="plan")]
    out.append("")
    return out


def render_directive_lines(rows: Optional[List[Dict]],
                           style: str = "plan") -> List[Tuple[str, str]]:
    """`(slug, line)` per directive: constraints first, then most-repeated first.

    THE ledger renderer (v2.16.0, B5). PLAN.md's section above and the
    SessionStart injection layer (`session_start._build_directives_layer`)
    each carried a loop of their own, and duplicate renderers of one ledger
    drift apart.
    `style` picks the surface: "plan" bolds the slug for a markdown file,
    "inject" brackets the kind for the one-line-per-row injection layer. The
    first tuple element is the RAW slug (the inject manifest records it);
    every rendered slot is `neutralize_inline`d — the text is user-authored
    through the CLI and escaped on the write path too, but rows written
    before both halves existed are already in users' databases.
    """
    rows = [r for r in (rows or []) if isinstance(r, dict)]
    rows = sorted(rows, key=lambda r: 0 if r.get("kind") == "constraint" else 1)
    out = []
    for r in rows:
        slug = neutralize_inline(str(r.get("slug") or ""))
        kind = neutralize_inline(str(r.get("kind") or "standing"))
        n = int(r.get("times_stated") or 1)
        demand = neutralize_inline(str(r.get("demand") or ""))
        quote = neutralize_inline(str(r.get("quote") or ""))
        if style == "inject":
            line = f"- [{kind}] {slug} (stated ×{n}): {demand}"
        else:
            line = f"- **{slug}** ({kind}, stated ×{n}): {demand}"
        if quote:
            line += f' — "{quote}"'
        out.append((str(r.get("slug") or ""), line))
    return out
