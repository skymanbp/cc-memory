...
from core.prompts import MEMORY_MD_FOOTER, MEMORY_MD_NOTICE
...
def _render_memory_index(db: MemoryDB, project_id: int, memory_dir: Path) -> str:
    ...
    # The notice is a `core.prompts` constant (v2.16.0, B7): three lines
    # where there were sixteen, because this file is read by the model.
    lines = [
        "# Memory Index",
        "",
        *MEMORY_MD_NOTICE,
    ...
    # No `python -m cc_memory.cli.mem …` tail (v2.16.0, B7): that spelling
    # runs in this repository and in no user project.
    lines += list(MEMORY_MD_FOOTER)
